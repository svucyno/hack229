  ...
  return (
    <div className="flex flex-col h-full">
      <div className="p-4 border-b border-navy-700 flex justify-between items-center bg-navy-900/50">
        <h2 className="font-sans font-bold text-sm tracking-widest text-gray-300">INCOMING ALERTS</h2>
        <span className="bg-emergency text-white text-xs font-bold px-2 py-0.5 rounded-full">{alerts.length}</span>
      </div>
      
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        {alerts.length === 0 ? (
          <div className="text-gray-500 font-mono text-xs text-center mt-10">No active alerts</div>
        ) : (
          alerts.map(alert => {
            ...
            return (
              <div 
                ...
                className={`
                  ...
                  ${isCritical ? 'border-emergency bg-navy-700 animate-pulse-border' : 
                    isModerate ? 'border-moderate bg-navy-700' : 'border-normal bg-navy-700'}
                  hover:bg-navy-600 shadow-sm
                `}
              >
                <div className="flex justify-between items-start mb-1">
                  <div className="font-bold font-sans text-sm">{alert.patient?.name || 'Unknown Protocol'}</div>
                  <div className="flex items-center text-gray-400 text-xs font-mono">
                    <Clock size={12} className="mr-1" /> Just now
                  </div>
                </div>
                <div className="font-mono text-xs text-red-200 mb-1">{alert.condition}</div>
