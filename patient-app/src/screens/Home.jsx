...
export default function HomeScreen({ go, language, updateLanguage }) {
  ...
  return (
    <div className="screen" style={{ paddingTop: 0, background: "#0D0D0D", display: "flex", flexDirection: "column" }}>
      {/* Header */}
      <div className="flex items-center justify-between" style={{ paddingTop: 52, paddingBottom: 8, paddingLeft: 20, paddingRight: 20 }}>
        <div className="flex items-center gap-2">
          ...
          <span className="font-syne" style={{ fontSize: 20, fontWeight: 800, letterSpacing: "-0.02em", color: "#fff" }}>MediRush</span>
        </div>
        ...
        <div style={{ position: "relative" }}>
          <button 
            ...
            style={{ padding: "6px 12px", borderRadius: 20, background: "#1A1A1A", border: "1px solid #2C2C2C", color: "#fff", fontSize: 13, display: "flex", alignItems: "center", gap: 6, cursor: "pointer" }}
          >
          ...
          {showLang && (
            <>
              ...
              <div style={{ position: "absolute", top: "110%", right: 0, width: 160, background: "#1A1A1A", border: "1px solid #2C2C2C", borderRadius: 12, overflow: "hidden", zIndex: 11, boxShadow: "0 10px 25px rgba(0,0,0,0.5)" }}>
                {LANGS.map(l => (
                  <button 
                    key={l.id}
                    onClick={() => { updateLanguage(l.id); setShowLang(false); }}
                    style={{ width: "100%", padding: "12px 16px", background: language === l.id ? "#2C2C2C" : "transparent", border: "none", color: "#fff", fontSize: 13, textAlign: "left", display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}
                  >
                    <span>{l.flag}</span>
                    <span>{l.name}</span>
                  </button>
      ...
      <div style={{ padding: "32px 24px 0", textAlign: "center" }}>
        <h1 className="font-syne" style={{ fontSize: 28, fontWeight: 800, color: "#fff", lineHeight: 1.2, marginBottom: 8 }}>{t.heading}</h1>
        <p className="font-dm" style={{ fontSize: 16, color: "#9E9E9E", lineHeight: 1.4 }}>{t.subheading}</p>
      </div>
      ...
      <div className="flex flex-col items-center justify-center" style={{ marginTop: 24, gap: 0 }}>
        <div style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "center", width: 220, height: 220 }}>
          ...
          <button className="sos-btn" onClick={() => go("input")} style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        <p className="font-dm" style={{ color: "#9E9E9E", fontSize: 15, marginTop: 24, textAlign: "center" }}>
          {t.desc}
        </p>
      </div>
      ...
      <div className="flex flex-col gap-3" style={{ marginTop: "auto", padding: "0 24px 32px" }}>
        <button className="btn-outline" onClick={() => go("records")} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, height: 52 }}>
        ...
        <button className="btn-outline-red" onClick={() => go("hospitals")} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, height: 52 }}>
